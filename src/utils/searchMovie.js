...
async function searchMovieYearByTitle(title) {
  try {
    // Search for movies with the given title
    const response = await fetch(`${baseUrl}/search/movie?api_key=${apiKey}&query=${title}`);
...
async function searchMovieDescriptionByTitle(title) {
  try {
    // Search for movies with the given title
    const response = await fetch(`${baseUrl}/search/movie?api_key=${apiKey}&query=${title}`);
...
export { searchMovieByTitle, searchMovieYearByTitle, searchMovieDescriptionByTitle };
